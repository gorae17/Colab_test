...
def test(testdata, trainedmodel):
  ...
  fscore = fbeta_score(np.array(y_test), y_pred, 0.5, average='micro')
  print(fscore)
                     
  
if __name__ == "__main__"
#  datapath = '/gdrive/My Drive/Colab Notebooks/AClassification.train.txt'
  datapath = sys.argv[1]
  df = init(datapath)

  traindata, testdata = train_test_split(df, random_state=random_state, test_size=0.3, shuffle=True)
  trainedmodel = train(traindata)
  test(testdata, trainedmodel)
